...
function foundation_scripts_and_styles() {
	if ( ! is_admin() ) {
		...
		wp_enqueue_script( 'foundation-tabs', get_template_directory_uri() . '/js/plugins/foundation.tabs.min.js', null, '6.3.0', true );
		wp_enqueue_script( 'foundation-accordion', get_template_directory_uri() . '/js/plugins/foundation.accordion.min.js', null, '6.3.0', true );
		//plugins
...
add_image_size( 'portfolio_thumb', 331, 331, array('center', 'center'));
...
add_filter('get_canonical_url','rewrite_canonical',10,2);

function rewrite_canonical($url, $post){
...
// Shortcode paste the Phone Number in Content

add_shortcode('phone', 'phone_shortcode');
function phone_shortcode( $atts, $content = null ) {
	if (!$content) {
		if (!($content = get_field('phone', 'options'))) {
			$content = null;
		}
	}
   return '<a class="phone" href="tel:'. preparePhone($content) . '"><i class="fa fa-mobile" aria-hidden="true"></i>'. $content .'</a>';
}




// Redirect single Portfolio link on Portfolio Page with # single ID

function my_page_template_redirect(){
	$page = get_field('portfolio_page', 'options');
	if( is_singular('fgs_portfolio') ){
		wp_redirect( $page .'#portfolio-'. get_the_ID() );
		exit();
	}
}
